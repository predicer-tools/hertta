...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == '/to_hertta/building_data':
            self.handle_building_data_request()
        else:
    ...
    def handle_building_data_request(self):
        # Construct an absolute path to the JSON file
        json_file_path = os.path.join(os.path.dirname(__file__), 'building_data.json')
        ...
        try:
            ...
            self.end_headers()
            self.wfile.write(json.dumps(building_data).encode())
        except FileNotFoundError:
            self.send_response(404)
            self.end_headers()
            self.wfile.write(b'Building data not found')
        except Exception as e:
            self.send_response(500)
            self.end_headers()
